...
import { Api, IEspecie, IEspecieCreate, IEspecieCurrent} from "../interfaces";
...
interface ApiCustom<T> extends Omit<Api<T>, "create" | "update"> {
  ...
  createGet: () => Promise<IEspecieCreate>;
  create?: (newData: IEspecieCurrent) => Promise<IEspecieCurrent>;
...
const EspecieApi: ApiCustom<IEspecie> = {
  ...
  createGet: async () => {
    const result = await API().get(Endpoints.ESPECIES_CREATE);
    return result.data;
  },
